...
import { parseCommand, parseResponse, type CommandApdu, type StatusKind } from './apdu'
import { detectProfile, type TlvProfile } from './tlv'
import { parseAtr } from './atr'
import { el, spaceHex, formatDelta } from './dom'
import {
  buildAtrCard,
  buildCommandCard,
  buildMalformedCard,
...
import { renderSummary, type LatencySample, type SummaryStats, type SummaryData } from './summary'
...
/** Snapshots the running aggregates for the summary renderer. */
...
function refreshSummary(): void {
  if (activeView === 'summary') renderSummary(summary, summaryData())
}
...
function selectView(name: ViewPreference, persist = true): void {
  ...
  if (name === 'summary') renderSummary(summary, summaryData())
